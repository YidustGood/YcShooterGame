...
public:
	...
	UFUNCTION()
	void OnSeamlessTravelStart(UWorld* World, const FString& MapUrl);
	...
	FGameplayMessageListenerHandle RegisterListener(
		...
		const UScriptStruct* StructType = TBaseStructure<FMessageStructType>::Get();
		return RegisterListenerInternal(Channel, ThunkCallback, StructType, MatchType, bUnregisterOnWorldDestroyed, UnregisterOnActorDestroyed);
	}
	...
	FGameplayMessageListenerHandle RegisterListener(
		...
		AActor* UnregisterOnActorDestroyed = nullptr;
		AActor* OwnerActor = Cast<AActor>(Object);
		if (IsValid(OwnerActor))
		{
			UnregisterOnActorDestroyed = OwnerActor;
		}
	...
	FGameplayMessageListenerHandle RegisterListener(
		...
		if (Params.OnMessageReceivedCallback)
		{
			...
			const UScriptStruct* StructType = TBaseStructure<FMessageStructType>::Get();
			Handle = RegisterListenerInternal(Channel, ThunkCallback, UnregisterOnActorDestroyed, StructType, Params.MatchType, bUnregisterOnWorldDestroyed);
		}
