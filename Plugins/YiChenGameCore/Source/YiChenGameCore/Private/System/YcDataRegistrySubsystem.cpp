...
bool UYcDataRegistrySubsystem::GetAllRegistryIds(FDataRegistryType RegistryType, TArray<FDataRegistryId>& OutIds)
{
	...
	if (!Registry)
	{
		UE_LOG(LogYcGameCore, Error, TEXT("UYcDataRegistrySubsystem::GetAllRegistryIds - Cannot find DataRegistry: %s"), 
...
void UYcDataRegistrySubsystem::RegisterDataTableToRegistry(FDataRegistryType RegistryType, TSoftObjectPtr<UDataTable> DataTable)
{
	...
	if (!Subsystem)
	{
		UE_LOG(LogYcGameCore, Error, TEXT("UYcDataRegistrySubsystem::RegisterDataTableToRegistry - DataRegistrySubsystem not available."));
		return;
	}
	
	if (DataTable.IsNull())
	{
		UE_LOG(LogYcGameCore, Warning, TEXT("UYcDataRegistrySubsystem::RegisterDataTableToRegistry - DataTable is null."));
		return;
	}
	...
	{
		UE_LOG(LogYcGameCore, Error, TEXT("UYcDataRegistrySubsystem::RegisterDataTableToRegistry - AssetManager not initialized."));
	}
}
...
bool UYcDataRegistrySubsystem::LoadDataRegistry(const TSoftObjectPtr<UDataRegistry>& RegistryPath)
{
	...
	if (!Subsystem)
	{
		UE_LOG(LogYcGameCore, Error, TEXT("UYcDataRegistrySubsystem::LoadDataRegistry - DataRegistrySubsystem not available."));
		return false;
	}
	
	if (RegistryPath.IsNull())
	{
		UE_LOG(LogYcGameCore, Warning, TEXT("UYcDataRegistrySubsystem::LoadDataRegistry - RegistryPath is null."));
		return false;
	}
...
UDataRegistrySubsystem* UYcDataRegistrySubsystem::GetEngineSubsystem()
{
	if (!GEngine)
	{
		UE_LOG(LogYcGameCore, Error, TEXT("UYcDataRegistrySubsystem::GetEngineSubsystem - GEngine is null."));
		return nullptr;
	}
	...
	if (!Subsystem)
	{
		UE_LOG(LogYcGameCore, Error, TEXT("UYcDataRegistrySubsystem::GetEngineSubsystem - DataRegistrySubsystem not available."));
	}
	return Subsystem;
}
